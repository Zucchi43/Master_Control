#Updated May 2024
#PID Control with ROS2 feedback for the closed loop
#Gabriel Zucchi
...
steer_pwm = lambda steer: (abs(steer)*0.6 + 40)
...
class SubscriberNode(Node):
    def __init__(self):
        ...
        self.speed_target=0
        self.steer_target =0
        self.Speed = Malha_PID(5,0.5,0.05,0)
        self.Steer = Malha_PID(4,0,0,2)
    ...
    def feedback_callback(self,msg):
        cur_speed = msg.linear.x
        cur_steer = msg.angular.z
        qual = msg.linear.z
        ts = time.time() - self.tempo
        if(qual > 0.5):self.Speed.update_malha(cur_speed,ts)
        else: self.Steer.update_malha(cur_steer,ts)
        self.tempo = time.time()

    def cmd_vel_callback(self, msg):
        self.speed_target = msg.linear.x
        self.steer_target = msg.angular.z
        self.Speed.update_target(speed=self.speed_target)
        self.Steer.update_target(steer=self.steer_target)
...
class Malha_PID():
    ...
    def update_malha(self,value,ts):
        self.atual = value
        if(self.id == 0):
...
def main(args=None):
    global control_array, limit, tempo
